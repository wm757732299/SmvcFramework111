	...
	var _cols = [
			...
			},
			{
				...
				formatter : [
						function uType(v, i) {
							var html =
//									'<button name ="deluser" class="btn btn-mini" onclick="javascript:UserList.delUser(\''
//									+ i
//									+ '\')"><i class="icon icon-trash" style="color:red">删除</i></button>'
//									+ 
...
//			custom : document.getElementById("ediMenuHtml") 
		...
		AjaxRequest.asyncAjaxPost(ediUrl, data, function(result) {
			if (result.success == "true") {
				_theEditMenuWindow = Dialog.dialog({
					...
					width : '700px',
					title : '编辑',
				//	remote :basePath + '/menu/edit_menu.wmctl?id='+id,
					moveable : true ,
				//	custom :document.getElementById("ediMenuHtml") 
					custom :function() {
						return addHmtl;
				...
				//==填充数据并展示编辑窗口==//
				_theEditMenuWindow.show({onShow: function() {
	...
	var _addSaveData = function(){
		var saveUrl="/menu/save_menu.wmctl";
		_saveData(saveUrl);
	};
	//==编辑保存节点==//
	var _editSaveData = function(){
		var saveUrl="/menu/edit_save.wmctl";
		_saveData(saveUrl);
	};
	//==保存节点==//
	var _saveData = function(urlType){
		$("#nodeType").removeAttr("disabled");
		var saveUrl = basePath + urlType;
		...
		AjaxRequest.asyncAjaxPost(saveUrl, formData, function(result) {
			...
			if (result.success == "true") {
				...
				if(_theSelectNode&&_theSelectNode.data.nodeType==0){
					_theSelectNode.data=data;
					 _treeObj.setting.callback.onClick(event, _zTreeId, _theSelectNode);
				}else{
					_theSelectNode.data.children.push(data);
					 _treeObj.setting.callback.onClick(event, _zTreeId, _theSelectNode);
				}
				if(_theAddMenuWindow) _theAddMenuWindow.close();
				if(_theEditMenuWindow) _theEditMenuWindow.close();
			} else {
				alert(result.msg);
			}
		});
	...
	var _addNode= function(){
		...
		if(nodes.length==0){
			_addMenuWindow();
			 ...
			 $("#nodeType").find("option[value = '1']").attr("selected","selected");
			...
			if("ONE-LEVEL-HOME-PAGE"==nodes[0].id){
				...
				if(nodes[0].data.nodeType==1){
					...
					$("#menuSort").val(nodes[0].children?(nodes[0].children.length+1):1);
				}else{
					alert("叶子节点下无法添加子节点!");
				}	
			}
	...
	var _showTree = function(t) {
		var setting = {
			async : {
				...
				autoParam : [ "id" ],
				type: "get"//默认post
				//otherParam: ["id", "1", "name", "test"]// 其他参数：将提交参数 id=1&name=test
			},
			callback: {
				...
				Dialog.delProDialog("该节点和其所有子节点都将删除，确定删除吗？",function() {
					...
					AjaxRequest.asyncAjaxPost(delUrl, delData, function(result) {
						if (result.success == "true") {
							...
							if(nextNode){
								for (var i = 0; theChildren&& i < theChildren.length; i++) {
									if(theChildren[i].id==theSelectNode.id){
										theChildren.splice(i,1);
									}
								}
								_treeObj.setting.callback.onClick(event, _zTreeId, nextNode);
							}else if(preNode){
								for (var j = 0; theChildren && j < theChildren.length; j++) {
									if(theChildren[j].id==theSelectNode.id){
										theChildren.splice(j,1);
									}
								}
								_treeObj.setting.callback.onClick(event, _zTreeId, preNode);
							}else if(parenNode){
								parenNode.data.children=[];
								_treeObj.setting.callback.onClick(event, _zTreeId, parenNode);//清空子节点
							}
						} else {
							alert(result.msg);
						}
